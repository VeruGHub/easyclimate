...
Here we extract different components of the date if needed:
...
mat_monthly_prcp <- mat_monthly_prcp |>
  mutate(month = factor(month, levels = rev(month_name)))

mat_prcp |> 
  mutate(month = factor(month, rev(month_name))) |> 
  ggplot(aes(x = month, y = Prcp, color = month)) +
  geom_jitter(size = 2, alpha = .3, width = .3, show.legend = FALSE) +
  geom_point( # add monthly data
    data = mat_monthly_prcp,
    aes(x = month, y = Prcp),
    color = "black", size = 3
  ) +
  scale_y_continuous(expand = c(0,0)) +
